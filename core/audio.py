...
class Audio():
    ...
    def __init__(self, name = None, datapath = None, labelpath = None, 
                 n_fft=1024, hop_length=1024, sr = 44100, y = None):
        self.name = name
        self.n_fft = n_fft
        ...
        self.S = librosa.amplitude_to_db(np.abs(librosa.stft(self.y, n_fft=n_fft, hop_length=hop_length, window=self.window_fn)))
        
    def wave_plot(self):
        librosa.display.waveshow(self.y, sr=self.sr)

    def freq_plot(self):
        frames = librosa.time_to_frames(np.linspace(0, librosa.get_duration(y=self.y, sr=self.sr), self.S.shape[1]),sr=self.sr, hop_length=self.hop_length)
        times = librosa.frames_to_time(frames, sr=self.sr, hop_length=self.hop_length)
        librosa.display.specshow(librosa.amplitude_to_db(self.S,ref=np.max), x_coords = frames, y_axis='log', hop_length=self.hop_length, ax=plt.gca())
        labels = list(zip(frames,times))
        labels = [f'({labels[j][0]}, \n {labels[j][1]:.2f})' for j in range(len(frames))]
        plt.xticks(frames,labels)
        plt.show()
    
    def time2frame(self, time):
        return librosa.time_to_frames(time, sr=self.sr, hop_length=self.hop_length)
    
    def frame2time(self,frame):
        return librosa.frames_to_time(frame, sr=self.sr, hop_length=self.hop_length)
    ...
    def print_info(self, start = 0, end = None):
        if end is None: end = self.S.shape[1]
    ...
    def get_whistle_events(self):
        if self.labelpath is None: return
