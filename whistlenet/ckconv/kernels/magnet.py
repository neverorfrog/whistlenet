...
from config.config import KernelConfig
from whistlenet.ckconv.layers import ckconv
...
class MAGNetLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        if causal and init_spatial_value != 1.0:
            mu = 1.0 - init_spatial_value * torch.rand(
        ...
        self.linear = Linear1d(data_dim, hidden_channels, bias=bias)
        ...
        self.filters = torch.nn.ModuleList(
            [
                MAGNetLayer(
                    ...
                    alpha=config.alpha / (layer + 1),
                    beta=config.beta,
                    bias=config.bias,
                    init_spatial_value=config.init_spatial_value,
                    causal=config.causal,
                )
                for layer in range(config.no_layers + 1)
            ]
        )
        # Initialize
...
def rotated_gaussian_window(x, gamma, theta, mu):
    return torch.exp(
        -0.5
        * ((gamma * rotate(2 * np.pi * theta, x.unsqueeze(1) - mu)) ** 2).sum(
            2
        )
    )
