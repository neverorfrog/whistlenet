...
from core.dataset import TensorData
...
class WhistleDataset(Dataset):
    """The Whistle dataset"""
    def __init__(self, tobeloaded: bool, params: dict, name = None, batch_size=64):
        self.save_parameters()
        self.classes = [0,1]
        if not tobeloaded:
            data, labels = self._get_data()
            train_data, train_labels, test_data, test_labels = self.split(data, labels, 0.2)
            train_data, train_labels, val_data, val_labels = self.split(train_data, train_labels, 0.2)
            ...
            super().__init__(tobeloaded, params, name = name, train_data=train_data, test_data=test_data, val_data=val_data)  
        else:
